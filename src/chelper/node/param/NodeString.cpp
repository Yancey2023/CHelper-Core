...
namespace CHelper::Node {
    ...
    NodeString::NodeString(const std::optional<std::wstring> &id,
    ...
    ASTNode NodeString::getASTNode(TokenReader &tokenReader, const CPack *cpack) const {
        ...
        if (HEDLEY_UNLIKELY(!allowMissingString && result.tokens.isEmpty())) {
            return ASTNode::simpleNode(this, result.tokens, ErrorReason::incomplete(result.tokens, L"字符串参数内容为空"));
        }
        if (HEDLEY_UNLIKELY(!canContainSpace)) {
            if (HEDLEY_UNLIKELY(result.tokens.toString().find(' ') != std::wstring::npos)) {
                return ASTNode::simpleNode(this, result.tokens, ErrorReason::contentError(result.tokens, L"字符串参数内容不可以包含空格"));
            }
            return result;
        }
        ...
        if (HEDLEY_UNLIKELY(!convertResult.isComplete)) {
            return ASTNode::simpleNode(this, result.tokens, ErrorReason::contentError(result.tokens, L"字符串参数内容双引号不封闭 -> " + std::wstring(str)));
        }
        return result;
    }
    ...
    void NodeString::collectStructure(const ASTNode *astNode,
        ...
        structure.append(isMustHave, description.value_or(L"字符串"));
    }
