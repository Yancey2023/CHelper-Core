    ...
    ASTNode NodeSingleSymbol::getASTNode(TokenReader &tokenReader, const CPack *cpack) const {
        ...
        if (HEDLEY_UNLIKELY(symbolNode.isError())) {
            if (HEDLEY_LIKELY(symbolNode.tokens.isEmpty())) {
                return ASTNode::simpleNode(this, symbolNode.tokens, ErrorReason::incomplete(symbolNode.tokens, fmt::format("命令不完整，需要符号{:c}", symbol)));
            } else {
                return ASTNode::simpleNode(this, symbolNode.tokens, ErrorReason::typeError(symbolNode.tokens, fmt::format("类型不匹配，需要符号{:c}，但当前内容为{}", symbol, symbolNode.tokens.toString())));
            }
        }
        ...
        return ASTNode::simpleNode(this, symbolNode.tokens, ErrorReason::contentError(symbolNode.tokens, fmt::format("内容不匹配，正确的符号为{:c}，但当前内容为{}", symbol, str)));
    }
