...
namespace CHelper::Node {
    ...
    static std::unique_ptr<NodeBase> nodeKey = std::make_unique<NodeNormalId>(
            ...
            std::make_shared<std::vector<std::shared_ptr<NormalId>>>(std::vector<std::shared_ptr<NormalId>>{
                    NormalId::make("item", "要检测的物品"),
                    NormalId::make("data", "要检测的物品的数据值"),
                    NormalId::make("quantity", "限制范围内的所有槽位中符合条件的物品的总和数量"),
                    NormalId::make("location", "要检测的物品栏"),
                    NormalId::make("slot", "用于指定要检测的槽位范围")}));
