...
namespace CHelper::Node {
    ...
    static std::shared_ptr<NodeBase> nodeEntry = std::make_shared<NodeEntry>(
            "BLOCK_STATE_ENTRY", "方块状态键值对", nodeEntryKey, nodeEntrySeparator, nodeEntryValue);
    static std::shared_ptr<NodeBase> nodeSeparator = std::make_shared<NodeSingleSymbol>(
            "BLOCK_STATE_SEPARATOR", "方块状态分隔符", ',');
    static std::shared_ptr<NodeBase> nodeRightBracket = std::make_shared<NodeSingleSymbol>(
